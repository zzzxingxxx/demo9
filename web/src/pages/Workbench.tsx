...
export function Workbench() {
  ...
  const projectMenuRef = useDismissableLayer(projectMenu, () =>
  ...
  async function refresh(preferredId?: string) {
    ...
    const next =
      data.projects.find(
        (p) => p.id === selectedId && (showArchived || !p.archived)
      ) ??
  ...
  useEffect(() => {
    ...
    apiGet<RulesPayload>(`/api/projects/${currentId}/rules`)
      ...
      .catch((err: unknown) => {
        if (!cancelled)
          setNotice(err instanceof Error ? err.message : "无法读取规则");
      });
    ...
    currentId,
    projects.find((p) => p.id === currentId)?.rootPath,
    setNotice,
    setRules,
    setTree
  ]);
  ...
  async function onCreate(event: FormEvent) {
    ...
    try {
      if (
        editingProject &&
        rootPath.trim() !==
          (projects.find((p) => p.id === editingProject)?.rootPath || "") &&
        useWorkbench.getState().tabs.some((t) => t.content !== t.original)
      )
        throw new Error("请先保存或关闭未保存文件，再更换绑定目录");
      const created = await apiSend<{ project: Project }>(
      ...
      if (
        ...
        localStorage.removeItem(`wb.agent.${editingProject}`);
      }
  ...
  async function newChat() {
    ...
    try {
      const created = await apiSend<{ session: { id: string } }>(
        "/api/sessions",
        "POST",
        { projectId: currentId }
      );
      if (useWorkbench.getState().currentId !== currentId) return;
  ...
  const sidebar = (
    <aside
      ...
      className={collapsed ? "gpt-sidebar is-collapsed" : "gpt-sidebar"}
      aria-label="项目与会话"
    >
      <div className="gpt-sidebar-head">
        {!collapsed && (
          <Link to="/" className="workbench-brand" aria-label="AI 工作台首页">
            <span className="brand-mark">
              <Command size={19} strokeWidth={1.8} />
            </span>
            <span>
              工作台<small>WORKBENCH</small>
            </span>
          </Link>
        )}
        <button
      ...
      <div className="sidebar-create">
        <button
          type="button"
          className={collapsed ? "gpt-icon" : "gpt-newchat"}
          title="新对话"
          aria-label="新对话"
      ...
      {collapsed && (
        <nav className="sidebar-rail" aria-label="常用功能">
          <button
            type="button"
            className={rightPanel === null ? "gpt-icon is-on" : "gpt-icon"}
            title="对话"
            aria-label="对话"
          ...
          <button
            type="button"
            className={isWorkspace ? "gpt-icon is-on" : "gpt-icon"}
            title="项目文件"
            aria-label="项目文件"
            onClick={() => openPanel("canvas")}
          >
            <FolderOpen size={18} />
          </button>
          <button
            type="button"
            className={
              rightPanel === "knowledge" ? "gpt-icon is-on" : "gpt-icon"
            }
            title="知识库"
            aria-label="知识库"
      ...
      {!collapsed && (
        <div className="gpt-sidebar-scroll">
          <div className="sidebar-section-label">
            ...
            <button
              type="button"
              className={showArchived ? "gpt-icon is-on" : "gpt-icon"}
              aria-label="显示归档项目"
              title="显示归档项目"
          ...
          <div className="gpt-pick">
            ...
            >
              {projects.length === 0 && (
                <option value="">选择或创建项目</option>
              )}
              {projects
                .filter(
                  (p) => showArchived || !p.archived || p.id === currentId
                )
                .map((project) => (
            ...
            <div className="ws-menu" ref={projectMenuRef}>
              ...
              {projectMenu && (
                <div id="project-actions" className="gpt-pop ws-pop">
                  ...
                  <button
                    ...
                    disabled={!current}
                    onClick={() =>
                      void projectAction(async () => {
                        if (!current) return;
                        setEditingProject(current.id);
                  ...
                  <button
                    ...
                    onClick={() =>
                      void projectAction(async () => {
                        if (!current) return;
                        await apiSend(`/api/projects/${current.id}`, "PATCH", {
                  ...
                  <button
                    ...
                    onClick={() =>
                      void projectAction(async () => {
                        if (
                          !current ||
                          !window.confirm(`删除项目 ${current.name}？`)
                        )
                          return;
          ...
          <nav className="sidebar-nav" aria-label="常用功能">
            <button
              type="button"
              className={rightPanel === null ? "active" : ""}
              onClick={() => openPanel(null)}
            >
            ...
            <button
              type="button"
              className={rightPanel === "knowledge" ? "active" : ""}
              onClick={() => openPanel("knowledge")}
            >
              <BookOpen size={17} />
              知识库
      ...
      <div className="gpt-sidebar-foot">
        <Link
          to="/settings"
          className={collapsed ? "gpt-icon" : "sidebar-settings"}
          title="设置"
          aria-label="设置"
        >
          <Settings size={17} />
  ...
  return (
    <main className="workbench">
      ...
      <div className="gpt-stage">
        <header className="gpt-top">
          ...
          <div className="workspace-breadcrumb">
            <Folder size={16} />
            <span className="gpt-top-title" title={current?.name}>
              {current?.name || "我的工作台"}
            </span>
            <ChevronRight size={13} className="breadcrumb-divider" />
            <span className="breadcrumb-page">
              {isWorkspace ? "项目文件" : tool?.label || "对话"}
            </span>
          </div>
          <div className="gpt-top-actions">
            <button
              type="button"
              className="gpt-icon header-search"
              title="搜索"
              aria-label="命令面板"
            ...
            <div className="ws-menu" ref={moreMenuRef}>
              <button
                type="button"
                className={showTool ? "gpt-icon is-on" : "gpt-icon"}
                title="工具"
                aria-label="工具"
                aria-expanded={moreMenu}
                aria-controls="workspace-tools"
                onClick={() => setMoreMenu((value) => !value)}
              >
                <LayoutGrid size={18} />
              </button>
              {moreMenu && (
                <div id="workspace-tools" className="gpt-pop ws-pop tools-pop">
                  <div className="menu-caption">工作台工具</div>
                  {MORE.map((item) => (
                    <button
                      key={item.id}
                      type="button"
                      className={rightPanel === item.id ? "active" : undefined}
            ...
            <button
              ...
              onClick={() => openPanel(isWorkspace ? null : "canvas")}
            >
              <PanelRight size={18} />
            </button>
          </div>
        ...
        <div className={rightPanel ? "gpt-body has-panel" : "gpt-body"}>
          ...
          />
          {isWorkspace && (
            <aside className="gpt-canvas" aria-label="工作区">
              <Canvas />
            </aside>
          )}
          {showTool && (
            <aside className="gpt-canvas" aria-label={tool?.label}>
              ...
              <div className="gpt-canvas-body" key={currentId}>
                  ...
                  (current ? (
                    <div className="bundle-actions">
                      <button
                        ...
                        onClick={() =>
                          void projectAction(async () => {
                            const data = await apiGet<{ json: string }>(
                              `/api/projects/${current.id}/export`
                            );
                      ...
                      <button
                        ...
                        onClick={() =>
                          void projectAction(async () => {
                            ...
                            if (!json) return;
                            await apiSend("/api/projects/import", "POST", {
                              json
                            });
                            await refresh();
                          })
      ...
      <Modal
        open={showNewProject}
        onClose={() => {
          if (!creating) setShowNewProject(false);
        }}
        label="新建项目"
      ...
      >
        ...
        <form className="stack project-form" onSubmit={onCreate}>
          ...
          <input
            ...
            placeholder="例如：个人网站、阅读笔记"
            required
            maxLength={120}
          />
          <label htmlFor="project-path">
            本地目录 <span className="optional-label">选填</span>
          </label>
          <input
            id="project-path"
            value={rootPath}
            onChange={(event) => setRootPath(event.target.value)}
            placeholder="输入本地文件夹的完整路径"
          />
          ...
          <div className="dialog-actions">
            ...
            >
              ...
              {creating ? "正在保存" : editingProject ? "保存项目" : "创建项目"}
            </button>
          </div>
        </form>
      </Modal>
