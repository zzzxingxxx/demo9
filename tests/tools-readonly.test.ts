...
import {
  ...
  executeReadFile,
  workbenchTools
} from "../server/src/lib/chat/tools.ts";

describe("read-only model tools", () => {
  const clean: string[] = [];
  afterEach(async () => {
    await Promise.all(
      clean.splice(0).map((p) => fs.rm(p, { recursive: true, force: true }))
    );
  });
  ...
  it("executeReadFile and executeListDir only read inside the bound root", async () => {
    ...
    await expect(executeReadFile(root, "../secret")).rejects.toThrow(
      /超出项目目录/
    );
    ...
    const withMcp = workbenchTools(root, [
      { name: "demo", command: "echo", args: [] }
    ]);
    expect(Object.keys(withMcp)).not.toContain("mcp_call");
  });
});
