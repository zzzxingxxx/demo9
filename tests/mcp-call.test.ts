...
describe("MCP call packing", () => {
  it("mcpJsonLine and parseMcpToolCallResult round-trip tool text", () => {
    expect(
      JSON.parse(
        mcpJsonLine(2, "tools/call", { name: "echo", arguments: { q: 1 } })
      )
    ).toEqual({
      jsonrpc: "2.0",
      id: 2,
  ...
  it("only discovered and explicitly allowed tools are exposed", () => {
    expect(Object.keys(mcpTools([]))).toEqual([]);
    expect(Object.keys(workbenchTools(".", []))).not.toContain("mcp_call");
    expect(
      Object.keys(mcpTools([{ name: "s", command: "echo", args: [] }]))
    ).toEqual([]);
    expect(
      Object.keys(
        mcpTools([
